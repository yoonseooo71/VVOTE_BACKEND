...
async function getComment(request, response) {
  try {
    ...
    const result = await databaseQuery(postQuery, [postId]);
    const data = result.rows.map((el) => {
      return { ...el, reply: [], likes: parseInt(el.likes) };
    });
...
async function commentWrite(request, response) {
  try {
    const { post_id, writer_id, writer, contents, parents_id } = request.body;
    ...
    const result = await databaseQuery(postQuery, [post_id, writer_id, writer, contents, parents_id]);
    if (parents_id === null) {
